...
def run_calibration(cap, bg_model, w, h, seconds=3):
    ...
    while time.time() - start < seconds:
        ...
        msg = "STEP OUT OF FRAME - CALIBRATING BACKGROUND"
...
def main():
    ...
    portal = PortalBox()
    hud = HUD(h, w, dev_str)
    if getattr(tracker, "hands", None) is None:
        portal.set_manual_box(w, h)
    ...
    if bg_model.buf:
        ...
        bg_model.ready = True
        if hasattr(seg, "set_background"):
            seg.set_background(bg_model.get())
    ...
    while True:
        ...
        if key in (ord('q'), 27):
            ...
            portal.invisible = not portal.invisible
            if portal.invisible:
                portal.set_manual_box(w, h)
        elif key == ord('b'):
            portal.toggle_manual_box(w, h)
        elif key == ord('s'):
