...
class SegmentationEngine:
    def __init__(self):
        ...
        if solutions is not None:
            self.seg = solutions.selfie_segmentation.SelfieSegmentation(model_selection=1)
        else:
            print("[WARN] mediapipe.solutions not found. Using OpenCV fallback mask.")
    ...
    def get_mask(self, frame_bgr):
        ...
        if self.seg is None:
            ...
            bg = self._bg.astype(np.uint8)
            diff = cv2.absdiff(frame_bgr, bg)
            gray = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)
            gray = cv2.GaussianBlur(gray, (7, 7), 0)
            _, hard8 = cv2.threshold(gray, 30, 255, cv2.THRESH_BINARY)
            kernel = np.ones((13, 13), np.uint8)
            hard8 = cv2.morphologyEx(hard8, cv2.MORPH_OPEN, kernel)
            hard8 = cv2.morphologyEx(hard8, cv2.MORPH_CLOSE, kernel, iterations=2)
            hard8 = cv2.dilate(hard8, kernel, iterations=1)
            mask = cv2.GaussianBlur(hard8.astype(np.float32) / 255.0, (11, 11), 0)
            if self._prev_mask is not None:
                mask = 0.7 * mask + 0.3 * self._prev_mask
            self._prev_mask = np.clip(mask, 0, 1).astype(np.float32)
...
class HandTracker:
    def __init__(self):
        ...
        if solutions is not None:
            ...
            print("[WARN] mediapipe.solutions not found. Hand gestures disabled; use SPACE.")
...
class PortalBox:
    ...
    def toggle_manual_box(self, w, h):
        if self.manual and self.active:
            self.manual = False
            self.active = False
            self.tl = None
            self.br = None
        else:
            self.set_manual_box(w, h)
    ...
    def render(self, frame, seg_mask, bg, all_points=None):
        ...
        if alpha > 0.01 and bg is not None:
            ...
            roi_bg   = bg
            if self.manual and self.active and self.tl and self.br:
                seg_mask = np.zeros((h, w), dtype=np.float32)
                x1, y1 = self.tl
                x2, y2 = self.br
                seg_mask[y1:y2, x1:x2] = 1.0
                seg_mask = cv2.GaussianBlur(seg_mask, (21, 21), 0)
        ...
        if self.active and self.tl and self.br:
            ...
            if self.manual:
                msg = "SPACE -> VANISH" if not self.invisible else "SPACE -> REAPPEAR"
            else:
...
class HUD:
    ...
    def draw(self, frame, portal, info: dict):
        ...
        cv2.putText(frame, "Ghost / Invisibility Mode",
